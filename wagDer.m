...
dy4 = ((1/12)*(F(x(1:n-1)-2*dx)) - (2/3)*F(x(1:n-1)-dx)+(2/3)*F(x(1:n-1)+dx)-(1/12)*F(x(1:n-1)+2*dx))./dx;
dy5 = ((-3/640)*(F(x(1:n-1)-2.5*dx)) + (25/384)*F(x(1:n-1)-1.5*dx)-(75/64)*F(x(1:n-1)-0.5*dx)+(75/64)*F(x(1:n-1)+0.5*dx)-(25/384)*F(x(1:n-1)+1.5*dx)+(3/640)*F(x(1:n-1)+2.5*dx))./dx
plot(x,F,xc,dy);
plot(xc,dy4,xc,dy5)
end